...
/* 2. The first player in any player array is the wicketkeeper and the others are field players.
For India (team 1) create one variable ('Ind') with the wicketkeeper name, (REST OPERATORS)*/

let [wk, ...fieldPlayer] = player1;
console.log(`'wicket keeper:${wk} "other Players:" ${fieldPlayer}`);

//3. Create an array 'allPlayers' containing all players of both teams (22 players) (SPread Opreator)
...
for (let [key, value] of game.Century.entries()) {
  console.log(`Century:${key}, player:${value}`);
}
...
for (let currItem in game.odds) {
  ...
  add = add + game.odds[currItem];
  i = i + 1;
}

console.log("Average:", Math.floor(sum / counter));
console.log("Average:", Math.floor(add / i));
